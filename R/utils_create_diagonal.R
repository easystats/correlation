...
.create_diagonal <- function(params) {
  ...
  if ("Group" %in% names(params)) {
    diagonal$Group <- unique(params$Group)[1]
  }
  ...
  if ("z" %in% names(params)) {
    diagonal$z <- Inf
  }
  if ("df" %in% names(params)) {
    diagonal$df <- unique(params$df)[1]
  }
  if ("df_error" %in% names(params)) {
    diagonal$df_error <- unique(params$df_error)[1]
  }
  if ("CI" %in% names(params)) {
    diagonal$CI <- unique(params$CI)[1]
  }
  ...
  if ("n_Obs" %in% names(params)) {
    diagonal$n_Obs <- unique(params$n_Obs)[1]
  }
  ...
  if ("ROPE_Percentage" %in% names(params)) {
    diagonal$ROPE_Percentage <- 0
  }
  if ("BF" %in% names(params)) {
    diagonal$BF <- Inf
  }
  if ("log_BF" %in% names(params)) {
    diagonal$log_BF <- Inf
  }
  if ("Prior_Distribution" %in% names(params)) {
    diagonal$Prior_Distribution <- unique(params$Prior_Distribution)[1]
  }
  if ("Prior_Location" %in% names(params)) {
    diagonal$Prior_Location <- unique(params$Prior_Location)[1]
  }
  if ("Prior_Scale" %in% names(params)) {
    diagonal$Prior_Scale <- unique(params$Prior_Scale)[1]
  }

  for (var in names(params)[!names(params) %in% names(diagonal)]) {
    if (length(unique(params[[var]])) > 1L) {
      insight::format_error(
        "Something's unexpected happened when creating the diagonal data. Please open an issue at https://github.com/easystats/correlation/issues"
      )
    }
