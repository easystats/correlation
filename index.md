...
[`summary`](https://rdrr.io/r/base/summary.html)`(``results``)`` ``## # Correlation Matrix (pearson-method)`` ``## `` ``## Parameter | Petal.Width | Petal.Length | Sepal.Width`` ``## -------------------------------------------------------`` ``## Sepal.Length | 0.82*** | 0.87*** | -0.12`` ``## Sepal.Width | -0.37*** | -0.43*** | `` ``## Petal.Length | 0.96*** | | `` ``## `` ``## p-value adjustment method: Holm (1979)`

Note that one can also obtain the full, **square** and redundant matrix
using:

[`summary`](https://rdrr.io/r/base/summary.html)`(``results``, redundant ``=`` ``TRUE``)`` ``## # Correlation Matrix (pearson-method)`` ``## `` ``## Parameter | Sepal.Length | Sepal.Width | Petal.Length | Petal.Width`` ``## ----------------------------------------------------------------------`` ``## Sepal.Length | | -0.12 | 0.87*** | 0.82***`` ``## Sepal.Width | -0.12 | | -0.43*** | -0.37***`` ``## Petal.Length | 0.87*** | -0.43*** | | 0.96***`` ``## Petal.Width | 0.82*** | -0.37*** | 0.96*** | `` ``## `` ``## p-value adjustment method: Holm (1979)`

[`library`](https://rdrr.io/r/base/library.html)`(`[`see`](https://easystats.github.io/see/)`)`` `` ``results`` ``%>%`` `` `[`summary`](https://rdrr.io/r/base/summary.html)`(``redundant ``=`` ``TRUE``)`` ``%>%`` `` `[`plot`](https://rdrr.io/r/graphics/plot.default.html)`(``)`

![](reference/figures/README-7-1.png)

## Correlation tests

The
[`cor_test()`](https://easystats.github.io/correlation/reference/cor_test.md)
function, for pairwise correlations, is also very convenient for making
quick scatter plots.

[`plot`](https://rdrr.io/r/graphics/plot.default.html)`(`[`cor_test`](https://easystats.github.io/correlation/reference/cor_test.md)`(``iris``, ``"Sepal.Width"``, ``"Sepal.Length"``)``)`
...
`iris`` ``%>%`` `` `[`correlation`](https://easystats.github.io/correlation/reference/correlation.md)`(``partial ``=`` ``TRUE``)`` ``%>%`` `` `[`summary`](https://rdrr.io/r/base/summary.html)`(``)`` ``## # Correlation Matrix (pearson-method)`` ``## `` ``## Parameter | Petal.Width | Petal.Length | Sepal.Width`` ``## -------------------------------------------------------`` ``## Sepal.Length | -0.34*** | 0.72*** | 0.63***`` ``## Sepal.Width | 0.35*** | -0.62*** | `` ``## Petal.Length | 0.87*** | | `` ``## `` ``## p-value adjustment method: Holm (1979)`
...
[`library`](https://rdrr.io/r/base/library.html)`(`[`see`](https://easystats.github.io/see/)`)`` ``# for plotting`` `[`library`](https://rdrr.io/r/base/library.html)`(`[`ggraph`](https://ggraph.data-imaginist.com)`)`` ``# needs to be loaded`` `` `[`plot`](https://rdrr.io/r/graphics/plot.default.html)`(`[`correlation`](https://easystats.github.io/correlation/reference/correlation.md)`(``mtcars``, partial ``=`` ``TRUE``)``)`` ``+`` `` `[`scale_edge_color_continuous`](https://ggraph.data-imaginist.com/reference/scale_edge_colour.html)`(``low ``=`` ``"#000004FF"``, high ``=`` ``"#FCFDBFFF"``)`

![](reference/figures/README-12-1.png)
...
`iris`` ``%>%`` `` `[`correlation`](https://easystats.github.io/correlation/reference/correlation.md)`(``partial ``=`` ``TRUE``, multilevel ``=`` ``TRUE``)`` ``%>%`` `` `[`summary`](https://rdrr.io/r/base/summary.html)`(``)`` ``## # Correlation Matrix (pearson-method)`` ``## `` ``## Parameter | Petal.Width | Petal.Length | Sepal.Width`` ``## -------------------------------------------------------`` ``## Sepal.Length | -0.17* | 0.71*** | 0.43***`` ``## Sepal.Width | 0.39*** | -0.18* | `` ``## Petal.Length | 0.38*** | | `` ``## `` ``## p-value adjustment method: Holm (1979)`
...
`iris`` ``%>%`` `` `[`correlation`](https://easystats.github.io/correlation/reference/correlation.md)`(``partial ``=`` ``FALSE``, multilevel ``=`` ``TRUE``)`` ``%>%`` `` `[`summary`](https://rdrr.io/r/base/summary.html)`(``)`` ``## Parameter | Petal.Width | Petal.Length | Sepal.Width`` ``## -------------------------------------------------------`` ``## Sepal.Length | 0.36*** | 0.76*** | 0.53***`` ``## Sepal.Width | 0.47*** | 0.38*** | `` ``## Petal.Length | 0.48*** | |`
