# correlation: Methods for correlation analysis

Lightweight package for computing different kinds of correlations, such
as partial correlations, Bayesian correlations, multilevel correlations,
polychoric correlations, biweight correlations, distance correlations
and more. Part of the 'easystats' ecosystem.

References: Makowski et al. (2020)
[doi:10.21105/joss.02306](https://doi.org/10.21105/joss.02306) .

## Details

`correlation`

## See also

Useful links:

- <https://easystats.github.io/correlation/>

- Report bugs at <https://github.com/easystats/correlation/issues>

## Author

**Maintainer**: Brenton M. Wiernik <brenton@wiernik.org>
([ORCID](https://orcid.org/0000-0001-9560-6336))

Authors:

- Brenton M. Wiernik <brenton@wiernik.org>
  ([ORCID](https://orcid.org/0000-0001-9560-6336))

- Dominique Makowski <dom.makowski@gmail.com>
  ([ORCID](https://orcid.org/0000-0001-5375-9967)) \[inventor\]

- Indrajeet Patil <patilindrajeet.science@gmail.com>
  ([ORCID](https://orcid.org/0000-0003-1995-6531))

- Daniel Lüdecke <d.luedecke@uke.de>
  ([ORCID](https://orcid.org/0000-0002-8895-3206))

- Mattan S. Ben-Shachar <matanshm@post.bgu.ac.il>
  ([ORCID](https://orcid.org/0000-0002-4287-4801))

- Rémi Thériault <remi.theriault@mail.mcgill.ca>
  ([ORCID](https://orcid.org/0000-0003-4315-6788))

Other contributors:

- Mark White <markhwhiteii@gmail.com> \[reviewer\]

- Maximilian M. Rabe <maximilian.rabe@uni-potsdam.de>
  ([ORCID](https://orcid.org/0000-0002-2556-5644)) \[reviewer\]
